...
Write-Title "Scheduler: activity feed"
# forcer un item minutely pour voir vite
$folderBody=@{ path="/var/log"; frequency="minutely" }
Invoke-WebRequest -Method POST -Uri "$API/monitoring/folders" -Headers $AUTH -Body ($folderBody | ConvertTo-Json -Depth 4) | Out-Null

Start-Sleep -Seconds 65
$act = Invoke-WebRequest -Uri "$API/activity" -Headers $AUTH
