...
#     type: str = "activity"  # "activity" | "alerts"

# class PurgeBody(BaseModel):
#     type: str               # "activity" | "alerts"
#     level: str | None = None
#     from_: str | None = None  # ISO (alias "from" côté client)
#     to: str | None = None

#     class Config:
#         fields = {"from_": "from"}  # accepte "from" dans le JSON

# def _parse_iso(s: str | None) -> datetime | None:
#     if not s: return None
#     # gère "YYYY-MM-DDTHH:MM" (datetime-local) et ISO avec 'Z'
#     try:
...
#     def keep(log):
#         # si parsing échoue → ne purge pas par prudence
#         if not log or log.get("level") == "RAW":
#             # purge par "contains/level" uniquement si niveau match
#             pass
#         # filtre niveau
#         if body.level and (log.get("level") or "").upper() != body.level.upper():
#             return True  # garder si niveau ne matche pas le critère
...
#             return True  # hors fenêtre → garder
#         if t_to and t and t > t_to:
#             return True  # hors fenêtre → garder

#         # sinon → supprimer cette ligne (ne pas garder)
#         return False
